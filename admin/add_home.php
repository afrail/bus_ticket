...
if(isset($_REQUEST['Update'])){
    $UpdateRecord=$_REQUEST['Update'];  
    $Show_Query="SELECT * FROM bus,schedules WHERE bus.bus_no=schedules.bus_no and schedules.schedule_id='$UpdateRecord'";
    $Execute=mysqli_query($link,$Show_Query);

 while($DataRows=mysqli_fetch_array($Execute,MYSQLI_ASSOC)){
           $BusNumber=$DataRows['bus_no'];  
...
$query="UPDATE  schedules SET d_from='$from', d_to='$to', d_time='$d_time', d_date='$d_date' WHERE schedule_id='$UpdateRecord'";
...
<div class="main" style="background-color:  #9a7d0a">
	<div class="container">
		...
		<tr>
			<td>Bus Number</td>
			<td><input type="text" name="busNumber" value="<?php echo $BusNumber; ?>" placeholder="BusNumber"></td>
		</tr>		
		<tr>		
		   <td> Bus Name</td>
		   <td><input type="text" name="busName" value="<?php echo $Busname;?>" placeholder=" BusName"></td>	
        </tr>
        <tr>		
		   <td> From</td>
		   <td><input type="text" name="from" value="<?php echo $From; ?>" placeholder="rom"></td>	
        </tr>
        <tr>		
		   <td> To</td>
        ...
        <tr>		
		   <td> fare</td>
		   <td><input type="text" name="fare" value="<?php echo $Fare; ?>" placeholder=" price"></td>	
        </tr>
        <tr>		
		   <td> Time</td>
		   <td><input type="text" name="d_time" value="<?php echo $Datetime; ?>" placeholder=" Time"></td>	
        </tr>
        <tr>		
		   <td>Date</td>
			<td>
